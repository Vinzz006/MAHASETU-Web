...
DEFAULT_DEPARTMENTS = [
    {
        ...
        "owner_contact": {
            "name": "Officer Patil",
            "email": "patil.identity@maharashtra.gov.in",
            "phone": "022-22001121",
        },
    ...
    },
    {
        ...
        "owner_contact": {
            ...
            "email": "deshmukh.welfare@maharashtra.gov.in",
            "phone": "022-22001122",
        },
        "health_status": "HEALTHY",
    },
    {
        ...
        "owner_contact": {
            "name": "Officer Joshi",
            "email": "joshi.employment@maharashtra.gov.in",
            "phone": "022-22001123",
        },
        "health_status": "HEALTHY",
    },
    {
        ...
        "owner_contact": {
            "name": "Senior Registrar Kulkarni",
            "email": "kulkarni.archives@maharashtra.gov.in",
            "phone": "022-22001124",
        },
        "health_status": "HEALTHY",
    },
    {
        ...
        "owner_contact": {
            "name": "Officer Gokhale",
            "email": "gokhale.revenue@maharashtra.gov.in",
            "phone": "022-22001125",
        },
        "health_status": "HEALTHY",
    },
...
DEFAULT_CONNECTORS = [
    {
        ...
        "connection_config": {
            "base_url": "http://127.0.0.1:8000/api/v1/integrations/DEPT_A",
            "timeout": 10,
        },
    ...
    },
    {
        ...
        "connector_type": "REST_JSON",
        "connection_config": {
            "base_url": "http://127.0.0.1:8000/api/v1/integrations/DEPT_B",
            "timeout": 10,
        },
    ...
    },
    {
        ...
        "connection_config": {
            "base_url": "http://127.0.0.1:8000/api/v1/integrations/LEGACY_01",
            "timeout": 15,
        },
    ...
    },
    {
        ...
        "connection_config": {
            "base_url": "http://127.0.0.1:8000/api/v1/integrations/DEPT_SOAP",
            "timeout": 20,
        },
        "auth_config": {"auth_type": "MUTUAL_TLS"},
        "request_mapping": {
            "root_element": "CitizenVerificationRequest",
            "namespace": "http://mahasetu.gov.in/interop",
        },
        "response_mapping": {
            "envelope_xpath": "//soap:Body/mh:CitizenVerificationResponse"
        },
...
class DepartmentRegistryService:
    ...
    def get_department(cls, db: Session, department_id: str) -> DepartmentRegistryEntry:
        cls.seed_defaults_if_empty(db)
        dept = (
            db.query(DepartmentRegistryEntry)
            .filter(DepartmentRegistryEntry.department_id == department_id)
            .first()
        )
        if not dept:
            raise HTTPException(
                status_code=404,
                detail=f"Department '{department_id}' not found in Department Registry",
            )
        return dept
    ...
    def register_department(
        ...
        dept = (
            db.query(DepartmentRegistryEntry)
            .filter(DepartmentRegistryEntry.department_id == dept_id)
            .first()
        )
        if not dept:
...
class ConnectorRegistryService:
    ...
    def get_connector(cls, db: Session, connector_id: str) -> ConnectorRegistryEntry:
        DepartmentRegistryService.seed_defaults_if_empty(db)
        conn = (
            db.query(ConnectorRegistryEntry)
            .filter(ConnectorRegistryEntry.connector_id == connector_id)
            .first()
        )
        if not conn:
            raise HTTPException(
                status_code=404,
                detail=f"Connector '{connector_id}' not found in Connector Registry",
            )
        return conn
    ...
    def register_connector(
        ...
        conn = (
            db.query(ConnectorRegistryEntry)
            .filter(ConnectorRegistryEntry.connector_id == conn_id)
            .first()
        )
        if not conn:
    ...
    def execute_connector_test(
        cls, db: Session, connector_id: str, sample_input: Any
    ) -> dict[str, Any]:
        """
        ...
        if c_type == "REST_JSON":
            if conn.department_id == "DEPT_A":
                canonical = DataTransformationEngine.to_canonical_from_dept_a(
                    sample_input if isinstance(sample_input, dict) else {}
                )
                target_payload = DataTransformationEngine.from_canonical_to_dept_a(
                    canonical
                )
            else:
                canonical = DataTransformationEngine.to_canonical_from_dept_b(
                    sample_input if isinstance(sample_input, dict) else {}
                )
                target_payload = DataTransformationEngine.from_canonical_to_dept_b(
                    canonical
                )
        elif c_type == "SOAP_XML":
            canonical = DataTransformationEngine.to_canonical_from_soap_xml(
                str(sample_input)
            )
            target_payload = DataTransformationEngine.from_canonical_to_soap_xml(
                canonical
            )
        elif c_type == "PIPE_DELIMITED":
            canonical = DataTransformationEngine.to_canonical_from_legacy(
                str(sample_input)
            )
            target_payload = DataTransformationEngine.from_canonical_to_legacy(
                canonical
            )
        else:
            canonical = DataTransformationEngine.to_canonical_from_csv(
                str(sample_input)
            )

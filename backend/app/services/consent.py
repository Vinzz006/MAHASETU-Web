...
class ConsentManager:
    ...
    def request_consent(
        ...
        if scope is None:
            scope = ["READ_IDENTITY_DEMOGRAPHICS", "READ_DOMICILE_STATUS", "READ_INCOME_BRACKET"]
    ...
    @classmethod
    def reject_consent(cls, db: Session, consent_id: str, citizen_id: str, reason: str = "Citizen declined consent") -> Consent:
        consent = db.query(Consent).filter(Consent.id == consent_id).first()
        if not consent:
            raise HTTPException(status_code=404, detail="Consent record not found")

        if consent.citizen_id != citizen_id:
            raise HTTPException(status_code=403, detail="Unauthorized: You cannot reject this consent")
    ...
    def revoke_consent(cls, db: Session, consent_id: str, citizen_id: str) -> Consent:
        ...
        if consent.citizen_id != citizen_id:
            raise HTTPException(status_code=403, detail="Unauthorized: You cannot revoke another citizen's consent")
    ...
    def validate_consent(cls, db: Session, application_id: str) -> bool:
        """Basic check that an active, AUTHORIZED consent exists before any data exchange."""
    ...
    def enforce_data_exchange(
        ...
        if not consent:
            cls._log_data_sharing(
                db, consent_id=None, application_id=application_id, citizen_id="UNKNOWN",
                req_dept=requesting_dept, rec_dept=receiving_dept, scope=requested_categories,
                purpose=purpose or "Unspecified", status="BLOCKED_NO_CONSENT",
                reason="No consent record found for application", ip=ip_address, corr_id=correlation_id
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        if consent.status != "AUTHORIZED":
            cls._log_data_sharing(
                db, consent_id=consent.id, application_id=application_id, citizen_id=consent.citizen_id,
                req_dept=requesting_dept, rec_dept=receiving_dept, scope=requested_categories,
                purpose=purpose or consent.purpose, status="BLOCKED_STATUS_INVALID",
                reason=f"Consent status is '{consent.status}' (must be AUTHORIZED)", ip=ip_address, corr_id=correlation_id
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        if exp and exp < now_naive:
            ...
            cls._log_data_sharing(
                db, consent_id=consent.id, application_id=application_id, citizen_id=consent.citizen_id,
                req_dept=requesting_dept, rec_dept=receiving_dept, scope=requested_categories,
                purpose=purpose or consent.purpose, status="BLOCKED_EXPIRED",
                reason="Consent has expired", ip=ip_address, corr_id=correlation_id
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        if purpose and consent.purpose:
            # If explicit purpose was supplied, check that it's related
            pass
        ...
        for req_cat in requested_categories:
            matched = any(req_cat.lower() in ac for ac in allowed_categories) or any(req_cat.lower() in sc for sc in allowed_scopes)
            # If not explicitly matched and categories list is non-empty
            if not matched and allowed_categories:
                cls._log_data_sharing(
                    db, consent_id=consent.id, application_id=application_id, citizen_id=consent.citizen_id,
                    req_dept=requesting_dept, rec_dept=receiving_dept, scope=requested_categories,
                    purpose=purpose or consent.purpose, status="BLOCKED_SCOPE_EXCEEDED",
                    reason=f"Requested category '{req_cat}' exceeds granted consent scope",
                    ip=ip_address, corr_id=correlation_id
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
        ...
        cls._log_data_sharing(
            db, consent_id=consent.id, application_id=application_id, citizen_id=consent.citizen_id,
            req_dept=requesting_dept, rec_dept=receiving_dept, scope=requested_categories,
            purpose=purpose or consent.purpose, status="ALLOWED",
    ...
    @classmethod
    def get_data_sharing_history(cls, db: Session, citizen_id: str) -> List[Dict[str, Any]]:
        logs = db.query(DataSharingLog).filter(
            DataSharingLog.citizen_id == citizen_id
        ).order_by(DataSharingLog.timestamp.desc()).all()

...
class PrometheusMetrics:
    def __init__(self):
        ...
        self._http_request_durations: Dict[str, Tuple[float, int]] = defaultdict(lambda: (0.0, 0))
    ...
    def record_request(self, method: str, path: str, status_code: int, duration_seconds: float):
        ...
        with self._lock:
            ...
            self._http_request_durations[clean_path] = (curr_sum + duration_seconds, curr_count + 1)
    ...
    def generate_exposition(self) -> str:
        ...
        lines.append("# HELP mahasetu_uptime_seconds Total application uptime in seconds")
        lines.append("# TYPE mahasetu_uptime_seconds gauge")
        lines.append(f"mahasetu_uptime_seconds {uptime:.2f}")

        # Cache metrics
        lines.append("# HELP mahasetu_cache_hits_total Total count of Redis/in-memory cache hits")
        lines.append("# TYPE mahasetu_cache_hits_total counter")
        lines.append(f"mahasetu_cache_hits_total {self._cache_hits}")

        lines.append("# HELP mahasetu_cache_misses_total Total count of Redis/in-memory cache misses")
        ...
        with self._lock:
            ...
            lines.append("# HELP http_request_duration_seconds HTTP request latencies in seconds")
            lines.append("# TYPE http_request_duration_seconds summary")
            for path, (sum_d, count) in sorted(self._http_request_durations.items()):
                lines.append(f'http_request_duration_seconds_sum{{handler="{path}"}} {sum_d:.4f}')
                lines.append(f'http_request_duration_seconds_count{{handler="{path}"}} {count}')

            # Department Transactions
            if self._dept_tx_total:
                lines.append("# HELP mahasetu_department_transactions_total Total cross-department transactions processed")
                ...
                for (dept, status), count in sorted(self._dept_tx_total.items()):
                    lines.append(f'mahasetu_department_transactions_total{{department="{dept}",status="{status}"}} {count}')

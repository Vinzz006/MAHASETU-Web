...
class MasterDataManagementService:
    ...
    @classmethod
    def calculate_match_confidence(cls, incoming: Dict[str, Any], candidate: CitizenMasterRecord) -> Tuple[float, Dict[str, Any]]:
        """
        ...
        # 1. Mobile Phone (0.35)
        inc_phone = re.sub(r"\D", "", incoming.get("phone") or incoming.get("mobile") or "")
        ...
        # 3. Name (0.20)
        inc_name = (incoming.get("name") or incoming.get("full_name") or incoming.get("citizen_name") or "").strip().lower()
        cand_name = (candidate.full_name or "").strip().lower()
        if inc_name and cand_name:
            if inc_name == cand_name:
                score += 0.20
                breakdown["name_match"] = {"weight": 0.20, "type": "EXACT", "matched": True}
            else:
                inc_tokens = set(inc_name.split())
                cand_tokens = set(cand_name.split())
                common = inc_tokens.intersection(cand_tokens)
                if common:
                    partial = 0.20 * (len(common) / max(len(inc_tokens), len(cand_tokens)))
                    score += partial
                    breakdown["name_match"] = {"weight": round(partial, 2), "type": "PARTIAL", "matched": True}
                else:
                    breakdown["name_match"] = {"weight": 0.0, "type": "NONE", "matched": False}
        else:
            breakdown["name_match"] = {"weight": 0.0, "matched": False}

        # 4. Date of Birth (0.15)
        inc_dob = (incoming.get("dateOfBirth") or incoming.get("dob") or incoming.get("date_of_birth") or "")[:10]
        ...
        inc_addr = incoming.get("address") if isinstance(incoming.get("address"), dict) else {}
        inc_dist = (inc_addr.get("district") or incoming.get("district") or "").strip().lower()
    ...
    def match_or_register_citizen(
        ...
        existing_mapping = db.query(IdentifierRegistry).filter(
        ...
        if existing_mapping:
            master = db.query(CitizenMasterRecord).filter(
                CitizenMasterRecord.master_id == existing_mapping.master_id
            ).first()
            return {
        ...
        name = (incoming.get("name") or incoming.get("full_name") or incoming.get("citizen_name") or "").strip()

        candidates = db.query(CitizenMasterRecord).filter(
            CitizenMasterRecord.status == "ACTIVE"
        ).all()

        best_score = 0.0
        best_candidate: Optional[CitizenMasterRecord] = None
        best_breakdown = {}
        ...
        if best_candidate and best_score >= cls.AUTO_MATCH_THRESHOLD:
            mapping = IdentifierRegistry(
                ...
                is_active=True,
                metadata_json={"match_score": best_score, "matched_at": utc_now().isoformat()}
            )
            db.add(mapping)
            ...
            create_audit_log(
                db, actor_id, "MDM_IDENTIFIER_LINKED", "MDM",
                metadata={"master_id": best_candidate.master_id, "source_system": source_system, "source_identifier": source_identifier, "score": best_score}
            )
        ...
        # 3B. Ambiguous Match (0.50 <= score < 0.85) -> Review Queue
            ...
            create_audit_log(
                db, actor_id, "MDM_AMBIGUOUS_MATCH_FLAGGED", "MDM",
                metadata={"review_id": review.id, "candidate_master_id": best_candidate.master_id, "score": best_score}
            )
        ...
        # 3C. No Match (< 0.50) -> Create New Golden Record
            ...
            addr = incoming.get("address") if isinstance(incoming.get("address"), dict) else {}
            district = addr.get("district") or incoming.get("district") or "Pune"

            new_master = CitizenMasterRecord(
                master_id=master_id,
                full_name=name or "New Citizen",
                date_of_birth=(incoming.get("dateOfBirth") or incoming.get("dob") or "1998-01-01")[:10],
            ...
            create_audit_log(
                db, actor_id, "MDM_MASTER_RECORD_CREATED", "MDM",
                metadata={"master_id": master_id, "source_system": source_system, "source_identifier": source_identifier}
            )
    ...
    def list_master_records(cls, db: Session, limit: int = 50) -> List[Dict[str, Any]]:
        records = db.query(CitizenMasterRecord).filter(
            CitizenMasterRecord.status == "ACTIVE"
        ).order_by(CitizenMasterRecord.created_at.desc()).limit(limit).all()
        return [cls._format_master_record(r) for r in records]

    @classmethod
    def get_master_record(cls, db: Session, master_id: str) -> Dict[str, Any]:
        record = db.query(CitizenMasterRecord).filter(
            CitizenMasterRecord.master_id == master_id
        ).first()
        if not record:
            raise HTTPException(status_code=404, detail=f"Citizen Master Record '{master_id}' not found")
        return cls._format_master_record(record)

    @classmethod
    def list_pending_reviews(cls, db: Session) -> List[Dict[str, Any]]:
        reviews = db.query(MDMMatchReview).filter(
            MDMMatchReview.status == "PENDING_REVIEW"
        ).order_by(MDMMatchReview.created_at.desc()).all()
        return [
            {
                "id": r.id,
    ...
    def resolve_review(
        ...
        if dec == "MERGE":
            ...
            candidate = db.query(CitizenMasterRecord).filter(
                CitizenMasterRecord.master_id == review.candidate_master_id
            ).first()
            if candidate:
                inc = review.incoming_record
                src_sys = inc.get("source_system", "DEPT_OFFICER_MERGE")
                src_id = inc.get("source_identifier") or f"MANUAL-{uuid.uuid4().hex[:6]}"
                mapping = IdentifierRegistry(
            ...
            new_master = CitizenMasterRecord(
                ...
                date_of_birth=(inc.get("dateOfBirth") or inc.get("dob") or "1998-01-01")[:10],
                primary_mobile=inc.get("phone") or inc.get("mobile") or "9999999999",
                primary_email=inc.get("email") or f"citizen_{master_id.lower()}@mahasetu.gov.in",
        ...
        create_audit_log(
            db, officer_id, f"MDM_REVIEW_{review.status}", "MDM",
            metadata={"review_id": review.id, "decision": review.status, "notes": notes}
        )
    ...
    def _format_master_record(cls, record: CitizenMasterRecord) -> Dict[str, Any]:
        return {
            ...
            "linked_identifiers": [
                ...
                for i in record.identifiers if i.is_active
            ],

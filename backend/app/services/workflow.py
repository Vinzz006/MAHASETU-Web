...
from backend.app.integrations import get_connector, DepartmentBFailureController
...
class WorkflowEngine:
    ...
    def advance_step(cls, db: Session, application_id: str, actor_id: str = "SYSTEM") -> Dict[str, Any]:
        ...
        canonical_model = {
            "citizen": {
                ...
                "name": citizen_data.get("name") or (app.citizen.name if app.citizen else "Demo Citizen"),
                "phone": citizen_data.get("mobile") or (app.citizen.mobile if app.citizen else "9999999999"),
        ...
        # Step 2: Department A Identity Verification
            ...
            create_audit_log(db, actor_id, "EXECUTE_STEP", "DEPT_A", app.id, {"verification_id": res.get("verification_id")})
        ...
        # Step 3: Department B Eligibility Verification (Handles failure simulation and retries)
            ...
            while attempt <= max_retries:
                try:
                    res = connector.verify_eligibility(canonical_model, retry_attempt=attempt)
                    success = True
                    break
            ...
            if not success:
                ...
                publish_event("INTEGRATION_FAILED", app.application_number, "DEPT_B", {"error": last_error, "retries": max_retries})
                create_audit_log(db, actor_id, "INTEGRATION_FAILED", "DEPT_B", app.id, {"retries": max_retries, "error": last_error})
            ...
            create_audit_log(db, actor_id, "EXECUTE_STEP", "DEPT_B", app.id, {"eligibility_status": res.get("eligibility_status")})
        ...
        # Step 4: Department C Approval & Sanction
            ...
            create_audit_log(db, actor_id, "APPROVAL_GRANTED", "DEPT_C", app.id, {"sanction_number": res.get("sanction_number")})
        ...
        # Step 5: Admin Review & Sanction Sign-off
            ...
            return {"step": step_name, "status": "COMPLETED", "result": {"department_id": "ADMIN", "decision": "APPROVE"}}
        ...
        # Step 6: Independent Auditor Review
            ...
            return {"step": step_name, "status": "COMPLETED", "result": {"department_id": "AUDIT", "decision": "CONFIRM"}}
    ...
    def admin_review(
        ...
        if not step:
            raise HTTPException(status_code=400, detail="ADMIN_REVIEW step not found for this application")
        ...
        if dec == "APPROVE":
            ...
            db.commit()
            publish_event("ADMIN_APPROVED", app.application_number, "ADMIN", {"comments": comments})
            create_audit_log(db, actor_id, "ADMIN_APPROVED", "ADMIN", app.id, {"comments": comments})
            return {"status": "SUCCESS", "decision": "APPROVE", "application_status": app.status}
        elif dec == "REWORK":
            ...
            app.current_department = "PORTAL"
            db.commit()
            publish_event("REWORK_REQUESTED", app.application_number, "ADMIN", {"rejection_reason": reason})
            create_audit_log(db, actor_id, "ADMIN_REWORK_REQUESTED", "ADMIN", app.id, {"rejection_reason": reason})
            return {"status": "SUCCESS", "decision": "REWORK", "application_status": app.status, "rejection_reason": reason}
        else:
            raise HTTPException(status_code=400, detail="Invalid decision. Must be 'APPROVE' or 'REWORK'.")
    ...
    def auditor_review(
        ...
        if not step:
            raise HTTPException(status_code=400, detail="AUDITOR_REVIEW step not found for this application")
        ...
        if dec == "CONFIRM":
            ...
            app.current_department = "PORTAL"
            db.commit()
            publish_event("AUDITOR_CONFIRMED", app.application_number, "AUDIT", {"comments": comments})
            create_audit_log(db, actor_id, "AUDITOR_CONFIRMED", "AUDIT", app.id, {"comments": comments})
            return {"status": "SUCCESS", "decision": "CONFIRM", "application_status": app.status}
        elif dec == "FLAG":
            step.status = "FLAGGED"
            step.details = {
                "title": "Independent Auditor Compliance Verification",
            ...
            db.commit()
            publish_event("AUDITOR_FLAGGED", app.application_number, "AUDIT", {"comments": comments})
            create_audit_log(db, actor_id, "AUDITOR_FLAGGED", "AUDIT", app.id, {"comments": comments})
            return {"status": "SUCCESS", "decision": "FLAG", "application_status": app.status}
        else:
            raise HTTPException(status_code=400, detail="Invalid decision. Must be 'CONFIRM' or 'FLAG'.")
    ...
    def resubmit_application(
        ...
        if app.status not in ["REWORK", "REWORK_REQUESTED"]:
            raise HTTPException(status_code=400, detail=f"Application is in status '{app.status}'. Only applications in REWORK status can be resubmitted.")
        ...
        admin_step = db.query(WorkflowStep).filter(
            WorkflowStep.application_id == app.id,
            WorkflowStep.step_name == "ADMIN_REVIEW"
        ).first()
        if admin_step:
        ...
        db.commit()
        publish_event("APPLICATION_RESUBMITTED", app.application_number, "PORTAL", {"comments": comments})
        create_audit_log(db, actor_id, "APPLICATION_RESUBMITTED", "APPLICATION", app.id, {"comments": comments})
        return {"status": "SUCCESS", "application_id": app.id, "application_status": app.status}

    @classmethod
    def run_full_pipeline(cls, db: Session, application_id: str, actor_id: str = "SYSTEM") -> Dict[str, Any]:
        """
        ...
        for _ in range(len(WORKFLOW_PIPELINE)):
            ...
            if step_res.get("status") in ["EXCEPTION", "ALREADY_COMPLETED", "REWORK", "FLAGGED"]:
                break
        return {"application_id": application_id, "history": results}

    @classmethod
    def retry_exception(cls, db: Session, application_id: str, actor_id: str = "OFFICER") -> Dict[str, Any]:
        """
        ...
        if not failed_step:
            return {"status": "NO_FAILED_STEP", "message": "No failed steps found to retry."}

        failed_step.status = "PENDING"
        db.commit()

        # Update previous failed transaction to RESOLVED if exists
        prev_txn = db.query(DepartmentTransaction).filter(

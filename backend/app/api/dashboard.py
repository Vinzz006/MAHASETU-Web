...
def _compute_dashboard_metrics(db: Session) -> DashboardMetricsResponse:
    ...
    exceptions_db = (
        ...
        .filter(DepartmentTransaction.status.in_(["FAILED", "RETRYING"]))
    ...
    if app_ids:
        app_rows = (
            db.query(Application.id, Application.application_number)
    ...
    telemetry_rows = (
        db.query(
            DepartmentTransaction.department_id,
            DepartmentTransaction.status,
            func.count(DepartmentTransaction.id),
        )
    ...
    dept_a_rate = (
        100.0
        if dept_a_txns == 0
        else round(((dept_a_txns - dept_a_errs) / dept_a_txns) * 100, 1)
    )
    ...
    if is_dept_b_down:
        ...
        dept_b_rate = round(
            min(
                88.4, ((dept_b_txns - max(1, dept_b_errs)) / max(1, dept_b_txns)) * 100
            ),
            1,
        )
        dept_b_latency = 310
        ...
        dept_b_rate = (
            100.0
            if dept_b_txns == 0
            else round(((dept_b_txns - dept_b_errs) / dept_b_txns) * 100, 1)
        )
        dept_b_latency = 185
    ...
    dept_c_rate = (
        100.0
        if dept_c_txns == 0
        else round(((dept_c_txns - dept_c_errs) / dept_c_txns) * 100, 1)
    )
    ...
    legacy_rate = (
        100.0
        if legacy_txns == 0
        else round(((legacy_txns - legacy_errs) / legacy_txns) * 100, 1)
    )
    ...
    overall_success = (
        100.0
        if total_txns == 0
        else round(((total_txns - total_errs) / total_txns) * 100, 1)
    )
    ...
    result = DashboardMetricsResponse(
        ...
        active_in_flight=db.query(Application)
...
def get_integrations_monitor(
    ...
    return {
        ...
        "summary": {
            "total_connectors": 4,
            "healthy": sum(
                1 for d in metrics.department_health if d.status == "HEALTHY"
            ),
            "degraded_or_failed": sum(
                1 for d in metrics.department_health if d.status != "HEALTHY"
            ),
...
def get_all_audit_logs(
    ...
    current_user: User = Depends(
        require_roles(["AUDITOR", "ADMIN", "SYSTEM_ADMIN", "OFFICER"])
    ),
):
...
def get_governance_analytics(
    ...
    funnel = [
        {"stage": "Submitted", "count": total, "pct": 100},
        {
            "stage": "In Progress",
            "count": in_progress,
            "pct": round((in_progress / max(1, total)) * 100, 1),
        },
        {
            "stage": "Sanctioned",
            "count": completed,
        ...
        },
        {
            "stage": "Action Reqd",
            "count": rework,
            "pct": round((rework / max(1, total)) * 100, 1),
        },
        {
            "stage": "Exception",
            "count": exception,
            "pct": round((exception / max(1, total)) * 100, 1),
        },
    ]
    ...
    authorized_consents = (
        db.query(Consent).filter(Consent.status == "AUTHORIZED").count()
    )
    ...
    sla_breaches = (
        ...
        .filter(Application.status != "COMPLETED", Application.created_at < cutoff)
        .count()
    )
    ...
    txns_last_hour = (
        db.query(DepartmentTransaction)
        .filter(DepartmentTransaction.created_at >= hour_ago)
        .count()
    )
    ...
    dept_stats_rows = (
        ...
        .filter(
            DepartmentTransaction.department_id.in_(
                ["DEPT_A", "DEPT_B", "DEPT_C", "LEGACY_01"]
            )
        )
        .group_by(DepartmentTransaction.department_id, DepartmentTransaction.status)
        .all()
        ...
        dept_analytics.append(
            {
                ...
                "status": (
                    "HEALTHY"
                    if rate >= 90
                    else ("DEGRADED" if rate >= 70 else "CRITICAL")
                ),
            }
        )
    ...
    top_districts = sorted(district_counts.items(), key=lambda x: x[1], reverse=True)[
        :5
    ]
    ...
    total_staff = (
        db.query(User).filter(User.role.in_(["OFFICER", "ADMIN", "AUDITOR"])).count()
    )
    ...
    result = {
        ...
        "platform_health": (
            "HEALTHY"
            if sla_breaches == 0
            else ("DEGRADED" if sla_breaches <= 2 else "AT_RISK")
        ),

...
@router.post("/definitions", dependencies=[Depends(require_roles(["ADMIN", "SYSTEM_ADMIN"]))])
...
def cancel_application_workflow(
    ...
    reason = req.get("reason", "Cancelled by applicant")
    actor = current_user.name if current_user else "CITIZEN"
    return WorkflowEngine.cancel_workflow(db, application_id, reason=reason, actor_id=actor)

@router.post("/{application_id}/escalate", dependencies=[Depends(require_roles(["OFFICER", "ADMIN", "SYSTEM_ADMIN"]))])
def escalate_application_workflow(
    ...
    reason = req.get("reason", "SLA breach threshold reached")
    actor = current_user.name if current_user else "OFFICER"
    return WorkflowEngine.escalate_workflow(db, application_id, reason=reason, actor_id=actor)

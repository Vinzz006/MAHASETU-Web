...
router = APIRouter(prefix="/api/pqc", tags=["Post-Quantum Cryptography (PQC) Transition Sandbox"])
...
def simulate_hybrid_handshake(req: HybridHandshakeRequest):
    ...
    classical_sig = hashlib.sha256(("CLASSICAL:ED25519:" + raw_packet).encode()).hexdigest()
    pqc_lattice_cipher = hashlib.sha512(("PQC:ML-KEM-768:LATTICE:" + raw_packet).encode()).hexdigest()

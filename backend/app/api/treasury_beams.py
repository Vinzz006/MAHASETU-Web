...
router = APIRouter(prefix="/api/treasury-beams", tags=["MahaNidhi — Treasury & BeAMS Liquidity Auditor"])
...
def get_treasury_liquidity_pulse():
    ...
    """
    total_sanction = sum(d["quarterly_sanction_cr"] for d in DEPARTMENTAL_BEAMS_ALLOCATIONS)
    total_disbursed = sum(d["disbursed_to_date_cr"] for d in DEPARTMENTAL_BEAMS_ALLOCATIONS)
    total_headroom = sum(d["available_headroom_cr"] for d in DEPARTMENTAL_BEAMS_ALLOCATIONS)
    ...
    return {
        ...
        "beams_macro_summary": {
            ...
            "statewide_burn_rate_pct": round((total_disbursed / total_sanction) * 100, 2)
        },
...
def reconcile_scheme_sanction(req: ReconcileSanctionRequest):
    ...
    dept = next((d for d in DEPARTMENTAL_BEAMS_ALLOCATIONS if d["department"] == req.department), DEPARTMENTAL_BEAMS_ALLOCATIONS[0])
    ...
    return {
        "status": "TREASURY_CLEARANCE_APPROVED" if has_sufficient_headroom else "HEADROOM_EXCEEDED",
        "reconciliation_decision": "AUTHORIZED_FOR_IMMEDIATE_DBT" if has_sufficient_headroom else "HELD_PENDING_SUPPLEMENTARY_GRANT",
        "department": req.department,
        "scheme_code": req.scheme_code,
        "requested_amount_cr": req.requested_amount_cr,
        "remaining_department_headroom_cr": round(dept["available_headroom_cr"] - (req.requested_amount_cr if has_sufficient_headroom else 0), 2),

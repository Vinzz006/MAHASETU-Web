...
router = APIRouter(
    prefix="/api/tender-shield", tags=["MahaTender — Municipal Tender Collusion Shield"]
)
...
SAMPLE_MUNICIPAL_TENDERS = [
    {
        ...
        "bidders": [
            {
                "bidder_name": "Apex MegaInfra Ltd",
                "bid_quote_cr": 139.8,
                "submission_ip": "115.112.45.10",
            },
            {
                "bidder_name": "Zenith Coastal Buildcon",
                "bid_quote_cr": 141.2,
                "submission_ip": "115.112.45.10",
            },
            {
                "bidder_name": "Sahyadri EPC Consortium",
                "bid_quote_cr": 142.0,
                "submission_ip": "115.112.45.12",
            },
            {
                "bidder_name": "National Highway Infra",
                "bid_quote_cr": 156.4,
                "submission_ip": "49.207.12.8",
            },
        ],
    },
    {
        ...
        "bidders": [
            {
                "bidder_name": "JalShakti CleanTech",
                "bid_quote_cr": 84.5,
                "submission_ip": "103.21.14.88",
            },
            {
                "bidder_name": "AquaPure Municipal Sol",
                "bid_quote_cr": 86.2,
                "submission_ip": "103.21.14.89",
            },
            {
                "bidder_name": "Deccan Environmental",
                "bid_quote_cr": 87.8,
                "submission_ip": "157.240.2.35",
            },
        ],
    },
]
...
def analyze_tender_bids_for_collusion(req: AnalyzeTenderRequest):
    ...
    tender = next(
        (t for t in SAMPLE_MUNICIPAL_TENDERS if t["tender_id"] == req.tender_id),
        SAMPLE_MUNICIPAL_TENDERS[0],
    )
    ...
    if has_ip_collision:
        anomalies.append(
            "IP Address Coincidence: Apex MegaInfra & Zenith Coastal submitted from identical subnet (115.112.45.10)."
        )
        anomalies.append(
            "Cover Bidding Pattern: Zenith Coastal bid quote is artificially priced +1.0% above Apex to simulate competitive bidding."
        )
        anomalies.append(
            "Interlocking Directorship: Common shadow director detected in MCA-21 registry filings."
        )
    ...
    return {
        ...
        "anomalies_detected": (
            ...
            if anomalies
                ...
                "All bidders show independent network footprints and competitive margins."
            ]
        ),
        "recommended_vigilance_action": (
            "ISSUE_ACB_INSPECTION_NOTICE"
            if has_ip_collision
            else "CLEAR_FOR_COMMERCIAL_OPENING"
        ),
        "timestamp": now_iso,
    }

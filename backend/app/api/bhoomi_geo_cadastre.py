...
router = APIRouter(
    prefix="/api/bhoomi-cadastre",
    tags=["MahaBhoomi — Satellite Geo-Cadastre & CRZ Validator"],
)
...
def verify_cadastral_polygon(req: VerifyParcelPolygonRequest):
    ...
    if is_coastal:
        ...
        restrictions.append(
            "Intersects CRZ-II Buffer: Requires MCZMA coastal clearance before construction."
        )
    elif is_forest:
        clearance_status = "RESTRICTED_FOREST_BUFFER"
        restrictions.append(
            "Proximity to Protected Forest: Environmental impact assessment required."
        )
    else:
        restrictions.append(
            "Zero environmental overlap: Suitable for unrestricted welfare disbursal."
        )

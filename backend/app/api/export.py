...
def export_applications_csv(
    ...
    writer.writerow([
        "Application Number", "Citizen Name", "Citizen Mobile", "Service Name",
        "Current Status", "Assigned Department", "Created At", "Last Updated At"
    ])

    for a in apps:
        c_name = (a.citizen_data or {}).get("name") or (a.citizen.name if a.citizen else "Citizen")
        c_mobile = (a.citizen_data or {}).get("mobile") or (a.citizen.mobile if a.citizen else "N/A")
...
def export_audit_logs_csv(
    ...
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(["Log ID", "Application ID", "Actor ID", "Action", "Resource", "Tamper Hash", "Timestamp"])

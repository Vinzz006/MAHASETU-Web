...
def mock_government_sso_login(req: SsoMockLoginRequest, db: Session = Depends(get_db)):
    ...
    persona_map = {
        "CITIZEN": {"username": "9999999999", "name": "Demo Citizen", "role": "CITIZEN", "dept": None},
        "OFFICER": {"username": "8888888888", "name": "Officer Sharma", "role": "OFFICER", "dept": "DEPT_A"},
        "AUDITOR": {"username": "6666666666", "name": "Auditor Kulkarni", "role": "AUDITOR", "dept": "AUDIT"},
        "SYSTEM_ADMIN": {"username": "7777777777", "name": "System Admin", "role": "SYSTEM_ADMIN", "dept": None},
        "DEPARTMENT_A": {"username": "5555555551", "name": "Dept A Officer", "role": "DEPARTMENT_A", "dept": "DEPT_A"},
        "DEPARTMENT_B": {"username": "5555555552", "name": "Dept B Officer", "role": "DEPARTMENT_B", "dept": "DEPT_B"},
        "DEPARTMENT_C": {"username": "5555555553", "name": "Dept C Officer", "role": "DEPARTMENT_C", "dept": "DEPT_C"},
    }
    ...
    session_token = create_access_token(data={"sub": user.id, "role": user.role, "name": user.name})

    create_audit_log(
        db, user.id, "SSO_LOGIN_SUCCESS", "AUTH_SSO",
        metadata={"provider": "GOV_OIDC_SSO", "username": username, "role": role, "assurance": claims.get("assurance_level")}
    )

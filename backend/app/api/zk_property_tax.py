...
router = APIRouter(prefix="/api/zk-taxation", tags=["MahaKar — Zero-Knowledge Property Tax & Stamp Duty"])
...
def assess_property_tax_and_stamp_duty(req: AssessDutyRequest):
    ...
    zone = next((z for z in READY_RECKONER_ZONES if z["zone_code"] == req.zone_code), READY_RECKONER_ZONES[0])

    minimum_benchmark_value = req.carpet_area_sqft * zone["ready_reckoner_rate_per_sqft"]
    taxable_consideration = max(minimum_benchmark_value, req.declared_transaction_value_inr)

    total_tax_rate = zone["stamp_duty_rate_pct"] + zone["metro_cess_pct"] + zone["local_body_tax_pct"]
    payable_stamp_duty = round(taxable_consideration * (total_tax_rate / 100.0), 2)
    annual_property_tax = round(taxable_consideration * 0.0035, 2) # 0.35% municipal capital tax

    zk_proof_payload = f"{req.zone_code}:{req.carpet_area_sqft}:{taxable_consideration}:{payable_stamp_duty}:{now_iso}"
    zk_proof_hash = f"0xZK-STAMP-{hashlib.sha256(zk_proof_payload.encode()).hexdigest()[:24]}"

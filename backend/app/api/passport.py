...
def get_service_passport_certificate(
    ...
    if "DEPT_B" in txn_map:
        ...
        dept_b_proof = {
            ...
            "status": "PASSED",
            "certificate_id": res.get(
                "eligibility_certificate_id", "MH-ELIG-2026-48210"
            ),
    ...
    return {
        ...
        "department_attestations": {
            ...
            "dept_b_eligibility": dept_b_proof
            or {"status": f"ELIGIBLE ({svc_info['dept_b_label']})"},
            "dept_c_approval": dept_c_proof
            or {"status": f"SANCTIONED ({svc_info['dept_c_label']})"},
        },
    }
...
def verify_service_passport_authenticity(
    id_or_hash: str, response: Response, db: Session = Depends(get_db)
):
    response.headers["Cache-Control"] = "public, max-age=60"
    app = (
        db.query(Application)
        .filter(
            (Application.application_number == id_or_hash)
            | (Application.id == id_or_hash)
        )
        .first()
    )
    ...
    citizen_data = app.citizen_data or {}
    raw_name = citizen_data.get("name") or (
...
def get_application_pdf_receipt(
    ...
    sha_hash = hashlib.sha256(
        f"{tracking_id}:{citizen_name}:{app.status}".encode()
    ).hexdigest()

...
CUSTOM_CONNECTORS: List[Dict[str, Any]] = [
    {
        ...
        "fields": ["khata_no", "owner_name", "taluka", "district", "land_area_hectares"]
    },
    {
        "id": "DEPT_RATION_PDS",
        ...
        "status": "SANDBOX_ACTIVE",
        "fields": ["ration_card_no", "head_of_family", "scheme_category", "monthly_quota"]
    }
]
...
def list_studio_connectors():
    base_connectors = [
        {"id": k, "name": v.department_name, "type": v.integration_type, "is_core": True}
        for k, v in get_all_connectors().items()
    ]
    return {
        "core_connectors": base_connectors,
        "studio_connectors": CUSTOM_CONNECTORS
    }
...
def test_custom_transformation(req: CustomConnectorTestRequest):
    ...
    name = payload.get("owner_name") or payload.get("head_of_family") or payload.get("citizen_name") or payload.get("name") or "Sample Citizen"

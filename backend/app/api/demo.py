...
def get_demo_status():
    return {
        ...
        "system_status": (
            "EXCEPTION_SIMULATION_ACTIVE"
            if DepartmentBFailureController.simulate_failure
            else "ALL_SYSTEMS_NOMINAL"
        ),
    }
...
@router.post(
    "/toggle-failure",
    dependencies=[Depends(require_roles(["OFFICER", "SYSTEM_ADMIN"]))],
)
def toggle_department_b_failure(enabled: bool | None = None):
    if enabled is not None:
        DepartmentBFailureController.simulate_failure = enabled
    else:
        DepartmentBFailureController.simulate_failure = (
            not DepartmentBFailureController.simulate_failure
        )

    status_str = (
        "ACTIVATED" if DepartmentBFailureController.simulate_failure else "DEACTIVATED"
    )
    publish_event(
        "DEMO_MODE_CHANGED",
        "SYSTEM",

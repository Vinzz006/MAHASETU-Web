...
class DepartmentRegisterRequest(BaseModel):
    ...
    owner_contact: dict[str, Any] | None = None
    ...
    return [
        {
            ...
            "last_sync_time": (
                d.last_sync_time.isoformat() if d.last_sync_time else None
            ),
        }
...
@router.post(
    "/departments", dependencies=[Depends(require_roles(["OFFICER", "SYSTEM_ADMIN"]))]
)
...
def test_connector_execution(
    connector_id: str, req: ConnectorTestRequest, db: Session = Depends(get_db)
):
    """
    ...
    """
    return ConnectorRegistryService.execute_connector_test(
        db, connector_id, req.sample_payload
    )
...
def toggle_connector(
    connector_id: str, req: ConnectorToggleRequest, db: Session = Depends(get_db)
):

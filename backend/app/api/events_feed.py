...
router = APIRouter(prefix="/api/events", tags=["Real-Time Interoperability Event Radar"])
...
def get_live_events_telemetry(limit: int = 25, db: Session = Depends(get_db)):
    ...
    for log in logs:
        ...
        p_hash = hashlib.sha256(f"{log.id}-{log.action}-{log.timestamp}".encode()).hexdigest()[:16]
    ...
    return {
        "nodes": [
            {"id": "CITIZEN_PORTAL", "label": "Citizen Portal", "type": "CLIENT", "x": 100, "y": 200},
            {"id": "MAHASETU_HUB", "label": "MahaSetu Hub", "type": "HUB", "x": 350, "y": 200},
            {"id": "DEPT_A_REST", "label": "Dept A (Identity)", "type": "DEPT_REST", "x": 600, "y": 80},
            {"id": "DEPT_B_JSON", "label": "Dept B (Eligibility)", "type": "DEPT_JSON", "x": 600, "y": 200},
            {"id": "DEPT_C_SANCTION", "label": "Dept C (Sanctions)", "type": "DEPT_SANCTION", "x": 600, "y": 320},
            {"id": "LEGACY_MAINFRAME", "label": "Legacy Mainframe", "type": "LEGACY", "x": 350, "y": 380}
        ],
...
def simulate_telemetry_pulse(db: Session = Depends(get_db)):
    ...
    actions = ["IDENTITY_VERIFIED", "CANONICAL_TRANSFORMED", "ELIGIBILITY_VERIFIED", "APPROVAL_STARTED"]

...
router = APIRouter(prefix="/api/green", tags=["MahaHarit — Green GovTech & Carbon Offset Ledger"])
...
def get_green_footprint_metrics():
    ...
    seal_hash = hashlib.sha256(f"MAHAHARIT-CARBON-OFFSET-{total_co2_kg}-{now_iso[:10]}".encode()).hexdigest()
...
def get_district_green_certificate(district_name: str):
    ...
    """
    target = next((d for d in DISTRICT_GREEN_METRICS if d["district"].lower() == district_name.lower()), None)
    if not target:

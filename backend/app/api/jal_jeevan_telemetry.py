...
router = APIRouter(prefix="/api/jal-jeevan", tags=["MahaJal — Jal Jeevan Aquifer Telemetry"])
...
def get_aquifer_sensors():
    ...
    """
    avg_mbgl = sum(s["groundwater_level_mbgl"] for s in AQUIFER_SENSORS) / len(AQUIFER_SENSORS)
    avg_lpcd = sum(s["tap_water_supply_lpcd"] for s in AQUIFER_SENSORS) / len(AQUIFER_SENSORS)
...
def dispatch_emergency_potable_water_tanker(req: DispatchTankerRequest):
    ...
    cluster = next((s for s in AQUIFER_SENSORS if s["sensor_id"] == req.sensor_id), AQUIFER_SENSORS[0])

...
from backend.app.firebase import get_signed_passport_url, upload_passport_to_storage
...
def list_all_citizens(
    ...
    query = (
        db.query(User).filter(User.role == "CITIZEN").order_by(User.created_at.desc())
    )
    total = query.count()
    ...
    profiles = (
        {
            p.user_id: p
            for p in db.query(ResidentProfile)
            .filter(ResidentProfile.user_id.in_(user_ids))
            .all()
        }
        if user_ids
        else {}
...
def _get_or_create_profile(db: Session, user_id: str) -> ResidentProfile:
    profile = (
        db.query(ResidentProfile).filter(ResidentProfile.user_id == user_id).first()
    )
    if not profile:
...
def update_my_profile(
    ...
    if req.marital_status is not None:
        profile.marital_status = req.marital_status
    if req.community_caste is not None:
        profile.community_caste = req.community_caste
    ...
    if req.division is not None:
        profile.division = req.division
    if req.taluk is not None:
        profile.taluk = req.taluk
    if req.zone is not None:
        profile.zone = req.zone
    if req.full_address is not None:
        profile.full_address = req.full_address
    ...
    if req.aadhaar_number is not None:
        ...
        profile.aadhaar_last_four = (
            clean_aadhaar[-4:] if len(clean_aadhaar) >= 4 else clean_aadhaar
        )
    if req.pan_number is not None:
        profile.pan_number = req.pan_number.strip().upper()
    if req.passport_status is not None:
        profile.passport_status = req.passport_status

    # Family
    if req.father_name is not None:
        profile.father_name = req.father_name
    if req.mother_name is not None:
    ...
    if req.guardian_name is not None:
        profile.guardian_name = req.guardian_name
    ...
    # Education
    if req.educational_qualification is not None:
        profile.educational_qualification = req.educational_qualification
    ...
    if req.ifsc is not None:
        profile.ifsc = req.ifsc.strip().upper()
    ...
    create_audit_log(
        ...
        metadata={
            ...
            "fields_updated": list(req.model_dump(exclude_unset=True).keys()),
        },
    )
...
def get_citizen_profile(
    ...
    if any(
        "income" in cat or "bank" in cat or "socio-economic" in cat

...
router = APIRouter(prefix="/api/police-cctns", tags=["MahaRakshak — Police CCTNS Citizen Digital Station"])
...
def file_lost_property_nc_report(req: FileLostPropertyRequest):
    ...
    qr_signature = f"0xPOLICE-CCTNS-{hashlib.sha256(sig_payload.encode()).hexdigest()[:24]}"

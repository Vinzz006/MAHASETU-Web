...
router = APIRouter(
    prefix="/api/war-room", tags=["Chief Minister's Executive War Room (MahaDrishti)"]
)
...
def get_executive_macro_pulse():
    ...
    avg_sla = round(
        sum(d["sla_compliance_pct"] for d in REGIONAL_GOVERNANCE_DIVISIONS)
        / len(REGIONAL_GOVERNANCE_DIVISIONS),
        2,
    )
...
def simulate_policy_shift(req: PolicyShiftRequest):
    ...
    projected_additional_beneficiaries = int(
        142000 * (req.income_ceiling_expansion_pct / 10.0)
    )
    projected_additional_fiscal_cr = round(
        projected_additional_beneficiaries * 0.015 * req.welfare_budget_multiplier, 2
    )

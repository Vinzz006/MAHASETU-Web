...
router = APIRouter(prefix="/api/voice-hotline", tags=["MahaSanvad — Dialectal Voice Hotline Agent"])
...
def converse_with_voice_hotline(req: HotlineConverseRequest):
    ...
    dialect = next((d for d in SUPPORTED_MARATHI_DIALECTS if d["dialect_code"] == req.dialect_code), SUPPORTED_MARATHI_DIALECTS[0])

...
router = APIRouter(
    prefix="/api/security", tags=["Zero-Trust Security & Cryptographic Audit"]
)
...
def get_security_audit_scorecard(db: Session = Depends(get_db)):
    ...
    consent_integrity_pct = (
        100.0
        if total_consents == 0
        else round((valid_consents / total_consents) * 100, 1)
    )
    ...
    system_seal = hashlib.sha256(
        f"MAHASETU-SEC-{now.strftime('%Y%m%d%H')}-{total_apps}-{total_txns}".encode()
    ).hexdigest()

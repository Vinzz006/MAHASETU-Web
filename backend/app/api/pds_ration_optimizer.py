...
router = APIRouter(prefix="/api/pds-ration", tags=["MahaAnna — PDS Ration Supply Chain Optimizer"])
...
def dispatch_grain_replenishment_convoy(req: DispatchReplenishmentRequest):
    ...
    fps = next((f for f in FAIR_PRICE_SHOPS if f["fps_id"] == req.fps_id), FAIR_PRICE_SHOPS[0])

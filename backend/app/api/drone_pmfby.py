...
router = APIRouter(prefix="/api/drone-pmfby", tags=["MahaPahani — Drone AI Crop Damage & PMFBY Settlement"])
...
def settle_drone_pmfby_claim(req: SettleClaimRequest):
    ...
    survey = next((s for s in SAMPLE_DRONE_SURVEYS if s["survey_id"] == req.survey_id), SAMPLE_DRONE_SURVEYS[0])
    ...
    return {
        ...
        "crop": survey["crop"],
        "ndvi_damage_delta": round(survey["pre_disaster_ndvi"] - survey["post_disaster_ndvi"], 3),

...
SCHEME_BASELINES = {
    "employment-support": {
        "base_beneficiaries": 18000,
        "benefit_amount_inr": 60000,
        "adapter_weight": 1.0,
    },
    "farmer-dbt": {
        "base_beneficiaries": 45000,
        "benefit_amount_inr": 12000,
        "adapter_weight": 1.4,
    },
    "urban-housing": {
        "base_beneficiaries": 8500,
        "benefit_amount_inr": 250000,
        "adapter_weight": 1.8,
    },
    "smart-ration": {
        "base_beneficiaries": 92000,
        "benefit_amount_inr": 4800,
        "adapter_weight": 0.9,
    },
}
...
def evaluate_policy_scenario(req: PolicySimulationRequest):
    baseline = SCHEME_BASELINES.get(
        req.scheme_id, SCHEME_BASELINES["employment-support"]
    )
    ...
    projected_beneficiaries = int(
        baseline["base_beneficiaries"] * income_multiplier * district_ratio
    )
    total_budget_outlay_cr = round(
        (projected_beneficiaries * baseline["benefit_amount_inr"]) / 10000000.0, 2
    )

    # Interoperability infrastructure load modeling
    daily_transactions = (
        projected_beneficiaries * 4
    )  # 4 department adapter steps per citizen
    peak_throughput_tps = round(
        daily_transactions / (8 * 3600), 2
    ...
    return {
        ...
        "infrastructure_capacity_impact": {
            ...
            "hub_capacity_utilization_pct": min(
                95.0, round(peak_throughput_tps * 6.5, 1)
            ),
            ...
            "adapter_load_breakdown": {
                ...
                "legacy_mainframe_pipe_stream": (
                    "16%" if req.include_legacy_sync else "0%"
                ),
            },
        },
        "policy_recommendation": (
            "OPTIMAL_FOR_DEPLOYMENT: System capacity easily handles this scenario within existing 72h SLA thresholds."
            if sla_breach_probability < 30
            else "CAUTION: High transaction concurrency may require scaling Dept B and Legacy 7/12 adapter pools to prevent queue delays."
        ),
    }

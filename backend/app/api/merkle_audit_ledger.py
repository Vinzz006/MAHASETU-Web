...
router = APIRouter(prefix="/api/merkle-ledger", tags=["MahaLekha — Zero-Trust Merkle Tree Audit Notary"])
...
SAMPLE_MERKLE_BLOCKS = [
    {
        ...
        "validator_nodes": ["NIC_MAHARASHTRA_01", "CAG_AUDIT_NOTARY_02", "HIGH_COURT_REGISTRY_03"]
    },
    {
        "block_number": 1041,
...
def verify_merkle_inclusion(req: VerifyInclusionRequest):
    ...
    target_block = next((b for b in SAMPLE_MERKLE_BLOCKS if b["block_number"] == req.block_number), SAMPLE_MERKLE_BLOCKS[0])
    ...
    return {
        ...
        "inclusion_proof_path": [
            {"level": 1, "sibling_direction": "RIGHT", "hash": f"0x{sibling_1[:32]}..."},

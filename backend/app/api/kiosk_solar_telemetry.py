...
router = APIRouter(
    prefix="/api/kiosk-solar",
    tags=["MahaUrja — Gramin Kiosk Solar Microgrid Telemetry"],
)
...
def optimize_kiosk_power_profile(req: OptimizePowerRequest):
    ...
    kiosk = next(
        (k for k in REMOTE_SOLAR_KIOSKS if k["kiosk_id"] == req.kiosk_id),
        REMOTE_SOLAR_KIOSKS[0],
    )

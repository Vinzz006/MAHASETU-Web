...
class IdentifierRegistry(Base):
    ...
    master_id = Column(
        String(36),
        ForeignKey("citizen_master_records.master_id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    source_system = Column(
        String(50), nullable=False, index=True
    )  # DEPT_A, DEPT_B, DEPT_C, LEGACY_01, CIVIL_REGISTRY
    source_identifier = Column(
        String(100), nullable=False, index=True
    )  # e.g. "A-12345", "B-77891", "LEG-991"
    identifier_type = Column(
        String(50), nullable=False
    )  # CITIZEN_ID, BENEFICIARY_ID, FOLIO_NO, RATION_ID, TAX_ID
...
class MDMMatchReview(Base):
    ...
    candidate_master_id = Column(
        String(36),
        ForeignKey("citizen_master_records.master_id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    )  # PENDING_REVIEW, RESOLVED_MERGE, RESOLVED_NEW, REJECTED
    reviewed_by = Column(String(100), nullable=True)
    reviewed_at = Column(DateTime, nullable=True)
    resolution_notes = Column(Text, nullable=True)
    created_at = Column(DateTime, default=utc_now, nullable=False)

    candidate_record = relationship(
        "CitizenMasterRecord", foreign_keys=[candidate_master_id]
    )

...
class Consent(Base):
    ...
    consent_number = Column(String(64), unique=True, nullable=False, index=True) # e.g. CON-2026-004821
    ...
    requested_by = Column(String(100), nullable=False) # e.g. "Employment Department (DEPT_C)"
    ...
    purpose = Column(String(255), nullable=False) # e.g. "Eligibility verification for Employment Support Scheme"
    data_categories = Column(JSON, nullable=False) # ["Identity information", "Address information", "Eligibility information"]
    scope = Column(JSON, nullable=True) # e.g. ["READ_DEMOGRAPHICS", "READ_INCOME_BRACKET"]
    status = Column(String(50), nullable=False, default="REQUESTED") # REQUESTED, AUTHORIZED, DECLINED, REVOKED, EXPIRED
...
class DataSharingLog(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    consent_id = Column(String(36), ForeignKey("consents.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    status = Column(String(50), nullable=False) # ALLOWED, BLOCKED_NO_CONSENT, BLOCKED_SCOPE_EXCEEDED, BLOCKED_EXPIRED

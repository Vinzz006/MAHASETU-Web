...
class DepartmentRegistryEntry(Base):
    ...
    )  # e.g. "DEPT_A", "DEPT_B", "DEPT_C", "LEGACY_01"
    name = Column(
        String(255), nullable=False
    )  # e.g. "Civil Registration & Identity Department"
    code = Column(
    ...
    api_base_url = Column(
        String(255), nullable=False
    )  # e.g. "http://dept-a.internal.gov.in"
    protocol = Column(
        String(50), default="REST_JSON", nullable=False
    )  # REST_JSON, SOAP_XML, PIPE_DELIMITED, CSV_STREAM, MOCK_SANDBOX
    auth_method = Column(
        String(50), default="OAUTH2_BEARER", nullable=False
    )  # OAUTH2_BEARER, API_KEY, MUTUAL_TLS, BASIC_AUTH
    schema_version = Column(String(32), default="v1.0", nullable=False)
    service_list = Column(
        JSON, nullable=False, default=list
    )  # ["identity_verification", "demographic_lookup"]
    owner_contact = Column(
        JSON, nullable=True
    )  # {"name": "Officer Sharma", "email": "sharma@dept.gov.in", "phone": "022-22001122"}
...
class ConnectorRegistryEntry(Base):
    ...
    connector_id = Column(
        String(50), primary_key=True
    )  # e.g. "CONN_DEPT_A_REST", "CONN_LEGACY_01_PIPE"
    name = Column(String(255), nullable=False)
    department_id = Column(
        String(50),
        ForeignKey("department_registry.department_id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    auth_config = Column(
        JSON, nullable=False, default=dict
    )  # {"auth_type": "API_KEY", "token_url": "..."}
    request_mapping = Column(
        JSON, nullable=False, default=dict
    )  # Field mapping rules from CDM to Department Native
    response_mapping = Column(
        JSON, nullable=False, default=dict
    )  # Field mapping rules from Department Native to CDM
